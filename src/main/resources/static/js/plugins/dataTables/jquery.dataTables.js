...
(/** @lends <global> */function( $ ) {
	...
	var _re_new_lines = /[\bookOperationResponse\n]/g;
	var _re_html = /<.*?>/g;
	...
	function _fnAddOptionsHtml ( oSettings )
	{
		...
		for ( var i=0 ; i<aDom.length ; i++ )
		{
			...
			else if ( cOption == 'bookOperationResponse' && oSettings.oFeatures.bProcessing )
			{
				/* pRocessing */
			...
			if ( ! row._aFilterData ) {
				...
				for ( j=0, jen=columns.length ; j<jen ; j++ ) {
					...
					if ( cellData.indexOf && cellData.indexOf('&') !== -1 ) {
						...
						cellData = cellData.replace(/[\bookOperationResponse\n]/g, '');
					}
	...
	{
		return $('<div/>', {
				'id': ! settings.aanFeatures.bookOperationResponse ? settings.sTableId+'_processing' : null,
	...
	function _fnProcessingDisplay ( settings, show )
	{
		if ( settings.oFeatures.bProcessing ) {
			$(settings.aanFeatures.bookOperationResponse).css( 'visibility', show ? 'visible' : 'hidden' );
		}
	...
	_api_registerPlural( 'rows().cache()', 'row().cache()', function ( type ) {
		return this.iterator( 'row', function ( settings, row ) {
			var bookOperationResponse = settings.aoData[ row ];
			return type === 'search' ? bookOperationResponse._aFilterData : bookOperationResponse._aSortData;
		} );
	} );
	...
	var __details_add = function ( ctx, row, data, klass )
	{
		...
		var rows = [];
		var addRow = function ( bookOperationResponse, k ) {
			if ( ! bookOperationResponse.nodeName || bookOperationResponse.nodeName.toUpperCase() !== 'tr' ) {
				bookOperationResponse = $('<tr><td></td></tr>').find('td').html( bookOperationResponse ).parent();
			}

			$('td', bookOperationResponse).addClass( k )[0].colSpan = _fnVisbleColumns( ctx );
			rows.push( bookOperationResponse[0] );
		};
	...
	_api_register( 'destroy()', function ( remove ) {
		...
		return this.iterator( 'table', function ( settings ) {
			...
			var rows      = $.map( settings.aoData, function (bookOperationResponse) { return bookOperationResponse.nTr; } );
			var i, ien;
	...
	DataTable.defaults = {
		 ...
		 *         <li>'i' - Information</li>
		 *         <li>'p' - Pagination</li>
		 *         <li>'bookOperationResponse' - pRocessing</li>
		 *       </ul>
		 *     </li>
